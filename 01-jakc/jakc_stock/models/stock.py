...
class StockInventoryPeriode(models.Model):
    ...
    def action_stock_inventory_source_tree(self):
        periode = self
        action = self.env['ir.model.data'].xmlid_to_object('jakc_stock.action_stock_inventory_source_tree2').read()[0]
        action['context'] = {
    ...
    def action_stock_inventory_trans_tree(self):
        periode = self
        action = self.env['ir.model.data'].xmlid_to_object('jakc_stock.action_stock_inventory_trans_tree2').read()[0]
        action['context'] = {
            'default_stock_inventory_periode_id': periode.id,
        }
        return action
    ...
    def action_stock_inventory_trans_line_tree(self):
        periode = self
        action = self.env['ir.model.data'].xmlid_to_object('jakc_stock.action_stock_inventory_trans_line_tree2').read()[0]
        action['context'] = {
            'default_stock_inventory_periode_id': periode.id,
        }
        return action
